...
def _eval_baseline_run(
    ...
    split = int(0.95 * len(corpus))
    eval_set = ByteDataset(corpus[split:], cfg.optim.seq_len)
    metrics = evaluate_baseline(model, cfg, eval_set, n_batches=n_batches, device=device, seed=cfg.seed + 7)
    return metrics.to_dict()
...
def run_sweep(
    ...
    *,
    ...
    if not is_torch_backend(backend):
        raise ValueError(f"sweep runner requires a torch backend, got {backend!r}")
