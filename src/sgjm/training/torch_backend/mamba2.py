...
def _ssd_chunk_torch(
    ...
    cumlog = torch.cumsum(A_log, dim=1)
    ...
    M = torch.exp(cumlog[:, :, None, :] - cumlog_start[:, None, :, :])  # [B, L, L, H]
    ...
    BX_C = BX_C.permute(0, 4, 1, 2, 3)  # [B, L_t, L_s, H, P]
    ...
    decay_to_end = torch.exp(cumlog[:, -1:, :] - cumlog_start)  # [B, L, H]
...
class Mamba2Block(nn.Module):
    ...
    def _causal_conv(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        T = x.shape[1]
        # Left-pad time dimension with (d_conv - 1) zeros
